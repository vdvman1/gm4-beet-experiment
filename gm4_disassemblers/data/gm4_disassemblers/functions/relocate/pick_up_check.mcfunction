# checks if the block is from this module and sets the specific relocator data
...
from base.helpers import translate_fallback_str

execute
    if score gm4_disassemblers load.status matches 1
    if entity @s[tag=gm4_disassembler]
    run function gm4_disassemblers:relocate/set_pick_up_data:
        data modify storage gm4_relocators:temp merge_data set value {custom_block:"gm4_disassembler",lore: translate_fallback_str("Disassembler",{"translate":"block.gm4.disassembler"},{"color": "gray", "italic": True})}
